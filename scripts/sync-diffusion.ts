...
const contentRoot = path.join(siteRoot, 'src/content/series/diffusion');
const assetRoot = path.join(siteRoot, 'public/images/diffusion');
...
const routeByBasename = new Map(
  [
    ...routes.chapters.map((chapter) => ({ series: routes.series, chapter })),
    ...bridgeRoutes.chapters.map((chapter) => ({ series: bridgeRoutes.series, chapter })),
  ].map(({ series, chapter }) => [path.basename(chapter.source), { series, chapter }]),
);
...
async function transformMarkdown(chapter: Chapter, source: string): Promise<string> {
  ...
  visit(tree, 'link', (node, index, parent) => {
    ...
    if (chapterTarget) {
      ...
      node.url = `/blog/${chapterTarget.series}/${chapterTarget.chapter.slug}/${anchor}`;
      return;
    }
  ...
  const publicBody = body
    .replace(/`?tutorial_ready`?/g, '正式版 v1.0')
    .replace(/`?source_verified(?:_bounded)?`?/g, '已完成来源核验')
    .replace(/`?notes_complete`?/g, '已完成资料整理')
    .replace(/^python diffusion\/code\/[^\n]+$/gm, '# 本地验证脚本暂未公开');
...
if (referencedAssets.size !== allowlist.assets.length) {
  ...
  throw new Error(`Diffusion allowlist contains unreferenced assets: ${unreferenced.join(', ')}`);
}
...
if (writeMode) {
  ...
  console.log(
    `Generated ${routes.chapters.length} Diffusion chapters and ${allowlist.assets.length} assets; manifest now contains ${entries.length} entries.`,
  );
} else {
  console.log(
    `Dry run: ${routes.chapters.length} Diffusion chapters and ${allowlist.assets.length} assets are ready.`,
  );
